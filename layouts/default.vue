<template>
  <Suspense>
    <div>
      ...
      <div
        id="bodyCodeConfig"
        v-html="dataConfig.script_in_after_open_body_tag"
      ></div>
      <slot></slot>
      <TheFooter
        :logo="dataConfig.logo_footer"
        :logoSocial="dataConfig.social"
      />
      <div
        id="lastBodyCodeConfig"
        v-html="dataConfig.script_in_before_close_body_tag"
      ></div>
    </div>
  </Suspense>
</template>
...
onMounted(() => {
  const installFacebookSdkScript = (d, s, id) => {
    ...
    js.id = id;
    js.src =
      "https://connect.facebook.net/vi_VN/sdk.js#xfbml=1&version=v21.0&appId=1438807890055526";
    js.async = true;
    js.defer = true;
